...
pub fn query_config(deps: Deps) -> StdResult<Binary> {
    ...
    let response = ConfigResponse {
        admin : config.admin,
        admin_balance : config.admin_balance,
    };
    to_binary(&response)
}

pub fn query_user_info(deps: Deps, user: String) -> StdResult<Binary> {
    let user_addr = deps.api.addr_validate(&user)?; // String -> Addr

    let user_state = match USER_STATES.may_load(deps.storage, user_addr)? {
        Some(state) => state,
        None => UserState::default(), //Ok(None)
    };

    let response = UserInfoResponse {
        user_balance  : user_state.user_balance,
    };
